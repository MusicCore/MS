package com.example.musicapi.common.provider;
...
//                点击数
    ...
    public String listSQL(SerchBean sb) {
        return new SQL() {
            {
                SELECT("*");
                FROM("music_score");
                if (null != sb.getCondition().getAsStringEmptyNull("title")){
                    WHERE("title like "+"'%"+sb.getCondition().getAsStringEmptyNull("title")+"%'");
                };
                ORDER_BY("create_time limit "+sb.getPageStart()+","+sb.getRows());
                ;////                修改时间
//                SET("update_time = now() ");
////                最后一次修改人
//                SET("last_author = #{lastAuthor} ");
